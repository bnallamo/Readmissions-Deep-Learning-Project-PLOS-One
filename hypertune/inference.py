...
for val_ind in range(n_val):
    split = GroupShuffleSplit(n_splits=1, test_size=0.15, random_state=24+val_ind)
    ind = split.split(train_df0, groups=train_df0.HOSP_NRD)
    ...
    hist = model.fit([DX1_array_trn, DX_mat_trn, PR_mat_trn, hosp_array_trn, other_mat_trn], y_trn, 
                     batch_size=batchsize, epochs=80, callbacks=[checkpoint, reduce_lr, earlystop], 
    ...
    checkpoint = AUCCheckPoint(filepath=model_path+'temp/{}/inference_temp2.h5'.format(job_index), validation_y=y_val, 
                                 validation_x=[DX1_array_val, DX_mat_val, PR_mat_val, hosp_array_val, other_mat_val])
    hist = model.fit([DX1_array_trn, DX_mat_trn, PR_mat_trn, hosp_array_trn, other_mat_trn], y_trn, 
                     batch_size=batchsize, epochs=30, callbacks=[checkpoint, reduce_lr, earlystop], 
                     validation_data=[[DX1_array_val, DX_mat_val, PR_mat_val, hosp_array_val, other_mat_val], y_val], 
                    verbose=2)
...
auc_df.to_csv(path+'cohorts30/{}/inference/auc_{}{}_{}.csv'.format(cohort, split_seed, train_idx, sample_seed), index=False)
...
recyc_df.to_csv(path+'cohorts30/{}/inference/recyc_pred_{}{}_{}.csv'.format(cohort, split_seed, train_idx, sample_seed), index=False)